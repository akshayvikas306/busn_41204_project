...
saveRDS(bor.sel, "imp_vars_d.rds")

bor.sel <- readRDS('imp_vars_d.rds') #file contains variable list
...
hyper_grid <- expand.grid(
  ...
  bag.fraction = c(.5, .65),  ##  percent of training data to sample for each tree
...
barplot(var_imp$Gain, names = var_imp$Feature, horiz = TRUE, las = 2,
        main = 'Predictors of Value - Defenders')
...
pd2 <- pdp::partial(
  xgb.fit.final,
  pred.var = 'defending_sliding_tackle',
  ice = T,
  center = T,
  plot = T,
  alpha = .1,
