...
hyper_grid <- expand.grid(
  ...
  n.minobsinnode = c(5, 10), ##  minimum number of observations required in each terminal node
  bag.fraction = c(.5, .8),  ##  percent of training data to sample for each tree
...
barplot(var_imp$Gain, names = var_imp$Feature, horiz= TRUE, las = 2, 
        main = 'Variable Importance for Attackers')
