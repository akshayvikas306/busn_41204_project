...
saveRDS(bor.sel, "imp_vars_m.rds")

bor.sel <- readRDS('imp_vars_m.rds') #file contains variable list
...
barplot(var_imp, names = names(var_imp), horiz = TRUE, las = 2,
        main = 'Predictors of Value - Midfielders')
...
pd1 <- partial(rf.fit.final, pred.var = "skill_ball_control", 
               pred.fun = pred_wrapper)
pd2 <- partial(rf.fit.final, pred.var = "dribbling", 
               pred.fun = pred_wrapper)
pd3 <- partial(rf.fit.final, pred.var = "passing", 
               pred.fun = pred_wrapper)
pd4 <- partial(rf.fit.final, pred.var = "age", 
               pred.fun = pred_wrapper)

gridExtra::grid.arrange(autoplot(pd1), autoplot(pd2), 
                        autoplot(pd3), autoplot(pd4),
                        ncol = 2)

#Finding over-valued & under-valued players
final_pred_train <- predict(rf.fit.final, data = train)$predictions
final_pred_test <- predict(rf.fit.final, data = test)$predictions
...
most_overrated <- ranks %>% arrange(rank_diff) %>% head(20)
most_underrated <- ranks %>% arrange(desc(rank_diff)) %>% head(20)
